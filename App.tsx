...
import type { Language, Theme, PageId, AppState, StoredModuleData, ModuleFormData, ActiveModalState, ResultModalPayload, ConfirmModalPayload, ErrorModalPayload, SemesterResult, AnnualResult, CalculationMethod, ComplexCalculationMethod, SuccessModalPayload, ModalType } from './types';
import { Language as LanguageEnum, Theme as ThemeEnum } from './types';
...
const PageContainer: React.FC<PageContainerProps> = ({ titleKey, onBack, children, t, language, hasFixedFooter, headerActions }) => {
  return (
    <div className={`page-content w-full max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 ${hasFixedFooter ? 'pb-32' : 'pb-8'}`}>
      ...
      <div className="mt-8 animation-running animate-fadeInUp">
        {children}
      </div>
    </div>
  );
...
const SettingItem: React.FC<SettingItemProps> = ({ titleKey, icon, children, t, language, initiallyOpen = false }) => {
  ...
  return (
    <div className="setting-item bg-surface dark:bg-surface-dark p-1 rounded-lg-custom mb-6 border border-stroke dark:border-stroke-dark shadow-sm dark:shadow-dark">
      ...
      <div className={`setting-content-wrapper ${isOpen ? 'open' : ''}`}>
...
const Modal: React.FC<ModalProps> = ({ onClose, titleKey, children, t, language, wide, fullScreen, headerActions, modalContentRef }) => {
  ...
  return (
    <div className="modal-backdrop fixed inset-0 bg-black/60 dark:bg-black/70 flex justify-center items-center z-[2000] p-4 animate-modal-backdrop" onClick={onClose}>
      <div 
        ref={modalContentRef}
        className={`modal-content bg-surface dark:bg-surface-dark rounded-lg-custom shadow-xl dark:shadow-dark_strong overflow-y-auto animation-running animate-modal-content ${modalSizeClasses} ${fullScreen ? 'flex flex-col' : 'max-h-[90vh]'}`}
...
const App: React.FC = () => {
  ...
  const handleModuleFormChange = <K extends keyof ModuleFormData>(field: K, value: ModuleFormData[K]) => {
    ...
    setModuleForm(prev => {
        ...
        if (field === 'tdGrade' || field === 'tpGrade' || field === 'examGrade') {
            ...
            // The rest of the logic uses the potentially truncated string
            const numValue = parseFloat(strValue);
            if (!isNaN(numValue) && numValue > 20) {
                finalValue = '20' as ModuleFormData[K];
            } else {
                finalValue = strValue as ModuleFormData[K]; // Use the processed string
            }
        }
  ...
  const renderMainPage = () => (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 sm:p-8 bg-background dark:bg-background-dark">
        <div className="page-content w-full max-w-2xl text-center animation-running animate-fadeInUp">
  ...
  const renderSettingsPage = () => {
    ...
    const requiredCreditsOptions = [ { labelKey: "set_credits_option_30", value: 30 }, { labelKey: "set_credits_option_45", value: 45 }];
    const themeOptions = [ 
      ...
      {activeModal.type === 'result' && activeModal.payload && (() => {
        ...
        {activeModal.type === 'statement' && lastSemesterResultRef.current && (
            <Modal 
                ...
                fullScreen
            >
                <div ref={statementImageRef} className="statement-capture-area bg-surface dark:bg-surface-dark p-4 sm:p-6 h-full flex flex-col">
                    ...
                    <div className="flex-grow overflow-y-auto relative">
                        ...
                        <img
                            ...
                            alt="logo"
                            className={`absolute z-0 w-24 h-24 object-contain opacity-[0.05] pointer-events-none bottom-4 ${appState.language === LanguageEnum.AR ? 'left-4' : 'right-4'}`}
                        />
                    </div>
